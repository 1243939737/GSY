...
# 非局部注意力模块（Non-Local Attention）
class NonLocalAttention(nn.Module):
    def __init__(self, in_channels):
        super(NonLocalAttention, self).__init__()
        self.in_channels = in_channels
        self.inter_channels = in_channels // 2
        self.conv_query = nn.Conv2d(in_channels, self.inter_channels, kernel_size=1)
        self.conv_key = nn.Conv2d(in_channels, self.inter_channels, kernel_size=1)
        self.conv_value = nn.Conv2d(in_channels, self.inter_channels, kernel_size=1)
        self.conv_out = nn.Conv2d(self.inter_channels, in_channels, kernel_size=1)

    def forward(self, x):
        batch_size, channels, height, width = x.size()
        query = self.conv_query(x).view(batch_size, self.inter_channels, -1)
        key = self.conv_key(x).view(batch_size, self.inter_channels, -1)
        value = self.conv_value(x).view(batch_size, self.inter_channels, -1)

        # 计算注意力
        attention = torch.matmul(query.transpose(1, 2), key) / (height * width)
        attention = torch.softmax(attention, dim=-1)
        out = torch.matmul(attention, value.transpose(1, 2)).transpose(1, 2)
...
class MobileViTBv3(nn.Module):
    def __init__(self, channel, dim, depth=2, kernel_size=3, patch_size=(2, 2), mlp_dim=int(64 * 2), dropout=0.1):
        ...
        self.non_local_attention = NonLocalAttention(channel)  # 加入非局部注意力
